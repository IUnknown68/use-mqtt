...
const isConnected = reactify((state) => state === ConnectionState.Connected);
const isBusy = reactify((state) => state > ConnectionState.Connected);
const getSubscription = reactify(
  (mqtt, topic, options) => mqtt.subscriptions.get(topic) || new Subscription(mqtt, topic, options)
);
const getSubscriptionValue = reactify((subscription) => subscription.ref.value);
const getSubscriptionError = reactify((subscription) => subscription.error.value);
class ReactiveMqtt {
  constructor(id) {
    this.id = id;
...
export { ConnectionState, DEFAULT_CLIENT_ID, test, useMqtt, useSubscription };
//# sourceMappingURL=use-mqtt.mjs.map
